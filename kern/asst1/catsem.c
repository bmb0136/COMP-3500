 ...
 */
static volatile const char *catNames[NCATS] = {"Charlie", "Cooper", "Chad",
                                               "Carson",  "Caleb",  "Claire"};
...
static void tryTakeKitchen(const char *animalName, /*bool*/ char isCat,
                           struct semaphore *queue, unsigned long *waiting);
 ...
 *
 * Arguments:
 *    (bool) char isCat: Is the animal calling this function a cat or a mouse?
 * True (non-zero) if cat, false (zero) if mouse unsigned long animalNumber: the
 * specific animal that is calling this function unsigned long animalCount: the
 * total number of animals of the caller's type struct semaphore *queue: the
 * queue for animals of the caller's type struct semaphore *otherQueue: the
 * queue for animals NOT of the caller's type unsigned long *waiting: the number
 * of animals of the caller's type unsigned long *otherWaiting: the number of
 * animals NOT of the caller's type
 *
 */
static void animalsem(/*bool*/ char isCat, unsigned long animalNumber,
                      int animalCount, struct semaphore *queue,
                      struct semaphore *otherQueue, unsigned long *waiting,
                      unsigned long *otherWaiting) {
  ...
  const char *animalName =
      isCat ? catNames[animalNumber] : mouseNames[animalNumber];
  kprintf("!!! %s Started\n", animalName);
...
/*
 * tryTakeKitchen(): Let somebody in if the kitchen is free. Handles the case
 * when no animals are in the kitchen (which is the case initially).
 *
 * Arguments:
 ...
 *    unsigned long *waiting: The number of animals of the caller's type that
 * are waiting to enter the kitchen
 */
static void tryTakeKitchen(const char *animalName, /*bool*/ char isCat,
                           struct semaphore *queue, unsigned long *waiting) {
  P(mutex);
...
/*
 * leaveKitchen(): Leaves the kitchen. Will let the opposite animal type in if
 * possible, then try its own type, and finally will free the kitchen otherwise.
 *
 * Arguments:
 *    const char *animalName: The name of the animal that called this function
 *    (bool) char isCat: Whether the caller is a cat or not
 *    struct semaphore *queue: The queue to signal animals of the caller's type
 *    struct semaphore *otherQueue: The queue to signal animals NOT of the
 * caller's type unsigned long *waiting: The number of animals of the caller's
 * type that are waiting to enter the kitchen unsigned long *otherWaiting: The
 * number of animals NOT of the caller's type that are waiting to enter the
 * kitchen
 */
static void leaveKitchen(const char *animalName, /*bool*/ char isCat,
                         struct semaphore *queue, struct semaphore *otherQueue,
...
static void catsem(void *unusedpointer, unsigned long catnumber) {
  (void)unusedpointer;
  animalsem(1, catnumber, NCATS, catsQueue, miceQueue, &catsWaiting,
            &miceWaiting);
}
...
static void mousesem(void *unusedpointer, unsigned long mousenumber) {
  (void)unusedpointer;
  animalsem(0, mousenumber, NMICE, miceQueue, catsQueue, &miceWaiting,
            &catsWaiting);
}
